...
`;R();let L={getUserById(e){R();let t=c().prepare("SELECT id, name, email, avatar_url, bio, password FROM users WHERE id = ?").get(e);return t?S(t):null},loginWithEmail(e,t){R();let r=c().prepare("SELECT id, name, email, avatar_url, bio, password FROM users WHERE email = ?").get(e.toLowerCase().trim());if(!r||r.password!==t)throw Error("Invalid email or password.");return S(r)},signup(e){R();let t=c(),r=e.email.toLowerCase().trim();if(t.prepare("SELECT id FROM users WHERE email = ?").get(r))throw Error("An account with this email already exists.");let a=g();return t.prepare(`INSERT INTO users (id, name, email, password, avatar_url, bio, created_at)
       ...
       WHERE id = ?`).run(a,s,i,o,JSON.stringify(n),JSON.stringify(u),p??null,_(),e.id),this.getPostById(e.id)},deletePost(e){R();let t=c();t.transaction(()=>{t.prepare("DELETE FROM comments WHERE post_id = ?").run(e),t.prepare("DELETE FROM posts WHERE id = ?").run(e)})()},getCommentsByPostId:e=>(R(),c().prepare(`SELECT id, post_id, user_id, user_name, user_avatar_url, content, timestamp
         ...
         FROM comments
         WHERE id = ?`).get(a))},deleteComment(e,t){R();let r=c();r.transaction(()=>{let a=r.prepare("SELECT id, post_id FROM comments WHERE id = ?").get(t);a&&a.post_id===e&&(r.prepare("DELETE FROM comments WHERE id = ?").run(t),r.prepare("UPDATE posts SET comment_count = MAX(comment_count - 1, 0) WHERE id = ?").run(e))})()}}},29294:e=>{"use strict";e.exports=require("next/dist/server/app-render/work-async-storage.external.js")},44870:e=>{"use strict";e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},63033:e=>{"use strict";e.exports=require("next/dist/server/app-render/work-unit-async-storage.external.js")},76760:e=>{"use strict";e.exports=require("node:path")},78335:()=>{},94962:(e,t,r)=>{"use strict";r.r(t),r.d(t,{patchFetch:()=>c,routeModule:()=>d,serverHooks:()=>E,workAsyncStorage:()=>l,workUnitAsyncStorage:()=>m});var a={};r.r(a),r.d(a,{POST:()=>p});var s=r(96559),i=r(48088),o=r(37719),n=r(32190),u=r(26346);async function p(e){try{let t=await e.json(),r=u.w.signup(t);return n.NextResponse.json({user:r},{status:201})}catch(e){return n.NextResponse.json({error:e.message||"Signup failed"},{status:400})}}let d=new s.AppRouteRouteModule({definition:{kind:i.RouteKind.APP_ROUTE,page:"/api/auth/signup/route",pathname:"/api/auth/signup",filename:"route",bundlePath:"app/api/auth/signup/route"},resolvedPagePath:"/home/ritvik/code/Blog-Sphere/src/app/api/auth/signup/route.ts",nextConfigOutput:"",userland:a}),{workAsyncStorage:l,workUnitAsyncStorage:m,serverHooks:E}=d;function c(){return(0,o.patchFetch)({workAsyncStorage:l,workUnitAsyncStorage:m})}},96487:()=>{}};var t=require("../../../../webpack-runtime.js");t.C(e);var r=e=>t(t.s=e),a=t.X(0,[447,580],()=>r(94962));module.exports=a})();