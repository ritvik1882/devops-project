...
`;R();let L={getUserById(e){R();let t=c().prepare("SELECT id, name, email, avatar_url, bio, password FROM users WHERE id = ?").get(e);return t?S(t):null},loginWithEmail(e,t){R();let r=c().prepare("SELECT id, name, email, avatar_url, bio, password FROM users WHERE email = ?").get(e.toLowerCase().trim());if(!r||r.password!==t)throw Error("Invalid email or password.");return S(r)},signup(e){R();let t=c(),r=e.email.toLowerCase().trim();if(t.prepare("SELECT id FROM users WHERE email = ?").get(r))throw Error("An account with this email already exists.");let a=g();return t.prepare(`INSERT INTO users (id, name, email, password, avatar_url, bio, created_at)
       ...
       WHERE id = ?`).run(a,s,i,o,JSON.stringify(n),JSON.stringify(u),d??null,_(),e.id),this.getPostById(e.id)},deletePost(e){R();let t=c();t.transaction(()=>{t.prepare("DELETE FROM comments WHERE post_id = ?").run(e),t.prepare("DELETE FROM posts WHERE id = ?").run(e)})()},getCommentsByPostId:e=>(R(),c().prepare(`SELECT id, post_id, user_id, user_name, user_avatar_url, content, timestamp
         ...
         FROM comments
         WHERE id = ?`).get(a))},deleteComment(e,t){R();let r=c();r.transaction(()=>{let a=r.prepare("SELECT id, post_id FROM comments WHERE id = ?").get(t);a&&a.post_id===e&&(r.prepare("DELETE FROM comments WHERE id = ?").run(t),r.prepare("UPDATE posts SET comment_count = MAX(comment_count - 1, 0) WHERE id = ?").run(e))})()}}},29294:e=>{"use strict";e.exports=require("next/dist/server/app-render/work-async-storage.external.js")},44870:e=>{"use strict";e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},63033:e=>{"use strict";e.exports=require("next/dist/server/app-render/work-unit-async-storage.external.js")},71776:(e,t,r)=>{"use strict";r.r(t),r.d(t,{patchFetch:()=>c,routeModule:()=>p,serverHooks:()=>E,workAsyncStorage:()=>m,workUnitAsyncStorage:()=>l});var a={};r.r(a),r.d(a,{GET:()=>d});var s=r(96559),i=r(48088),o=r(37719),n=r(32190),u=r(26346);async function d(e,t){let{id:r}=await t.params,a=u.w.getUserById(r);return a?n.NextResponse.json({user:a}):n.NextResponse.json({error:"User not found"},{status:404})}let p=new s.AppRouteRouteModule({definition:{kind:i.RouteKind.APP_ROUTE,page:"/api/auth/session/[id]/route",pathname:"/api/auth/session/[id]",filename:"route",bundlePath:"app/api/auth/session/[id]/route"},resolvedPagePath:"/home/ritvik/code/Blog-Sphere/src/app/api/auth/session/[id]/route.ts",nextConfigOutput:"",userland:a}),{workAsyncStorage:m,workUnitAsyncStorage:l,serverHooks:E}=p;function c(){return(0,o.patchFetch)({workAsyncStorage:m,workUnitAsyncStorage:l})}},76760:e=>{"use strict";e.exports=require("node:path")},78335:()=>{},96487:()=>{}};var t=require("../../../../../webpack-runtime.js");t.C(e);var r=e=>t(t.s=e),a=t.X(0,[447,580],()=>r(71776));module.exports=a})();