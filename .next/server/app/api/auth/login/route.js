(()=>{var e={};e.id=758,e.ids=[758],e.modules={3295:e=>{"use strict";e.exports=require("next/dist/server/app-render/after-task-async-storage.external.js")},10846:e=>{"use strict";e.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},26346:(e,t,r)=>{"use strict";r.d(t,{w:()=>L});let a=require("node:crypto");var i=r.n(a);let s=require("node:fs");var o=r.n(s),n=r(76760),u=r.n(n);let l=require("better-sqlite3");var d=r.n(l);let m=u().join(process.cwd(),"data"),p=u().join(m,"blog-sphere.sqlite"),E=e=>{e.pragma("journal_mode = WAL"),e.pragma("foreign_keys = ON"),e.exec(`
  ...
  `)},c=()=>{if(global.__blogSphereDb)return global.__blogSphereDb;o().mkdirSync(m,{recursive:!0});let e=new(d())(p);return E(e),global.__blogSphereDb=e,e},T="seeded-v1",_=()=>new Date().toISOString(),g=()=>i().randomUUID(),N=e=>{try{let t=JSON.parse(e);return Array.isArray(t)?t.map(String):[]}catch{return[]}},S=e=>({id:e.id,name:e.name,email:e.email,avatarUrl:e.avatar_url??void 0,bio:e.bio??void 0}),h=e=>({id:e.id,title:e.title,excerpt:e.excerpt,content:e.content,authorId:e.author_id,author:{id:e.author_id,name:e.authorName??"Unknown Author",avatarUrl:e.authorAvatarUrl??void 0,bio:e.authorBio??void 0},timestamp:e.timestamp,categories:N(e.categories),tags:N(e.tags),imageUrl:e.image_url??void 0,commentCount:e.comment_count,status:e.status}),O=e=>({id:e.id,postId:e.post_id,user:{id:e.user_id,name:e.user_name,avatarUrl:e.user_avatar_url??void 0},content:e.content,timestamp:e.timestamp}),R=()=>{let e=c(),t=e.prepare("SELECT value FROM meta WHERE key = ?").get(T);if(t?.value==="true")return;let r=Date.now(),a=[{id:"demo-alex",name:"Alex Rivera",email:"alex@blogsphere.local",password:"password123",avatar_url:null,bio:"Writes about product design and creative workflows.",created_at:new Date(r-2592e6).toISOString()},{id:"demo-sam",name:"Sam Carter",email:"sam@blogsphere.local",password:"password123",avatar_url:null,bio:"Frontend engineer sharing practical web experiments.",created_at:new Date(r-216e7).toISOString()}],i=[{id:"post-offline-1",title:"Building A Calm Offline Writing Routine",excerpt:"How I moved my writing flow to an always-available offline stack.",content:"Offline-first writing changed how quickly I can move from idea to draft.\\n\\n- I keep a weekly note queue.\\n- I expand one note into a post every morning.\\n- I publish when the piece feels useful, not perfect.",author_id:"demo-alex",status:"published",timestamp:new Date(r-1296e5).toISOString(),categories:JSON.stringify(["Productivity","Writing"]),tags:JSON.stringify(["offline","workflow","writing"]),image_url:null},{id:"post-offline-2",title:"A Tiny UI Checklist For Better Readability",excerpt:"Three fast checks I run before shipping any content-heavy page.",content:"When readability is good, users stay longer and understand more.\\n\\n1. Raise line-height on long paragraphs.\\n2. Keep headline rhythm consistent.\\n3. Use muted color only for secondary text.",author_id:"demo-sam",status:"published",timestamp:new Date(r-432e5).toISOString(),categories:JSON.stringify(["Frontend"]),tags:JSON.stringify(["ui","accessibility","typography"]),image_url:null},{id:"post-offline-draft-1",title:"Draft: Better Comment Moderation Patterns",excerpt:"Notes for a future article about low-friction moderation.",content:"Outline:\\n\\n- moderation cues\\n- local-first tooling\\n- transparent actions",author_id:"demo-alex",status:"draft",timestamp:new Date(r-144e5).toISOString(),categories:JSON.stringify(["Community"]),tags:JSON.stringify(["moderation","comments"]),image_url:null}],s=[{id:"comment-1",post_id:"post-offline-1",user_id:"demo-sam",user_name:"Sam Carter",user_avatar_url:null,content:"Love this. The note queue idea is easy to start today.",timestamp:new Date(r-72e6).toISOString()},{id:"comment-2",post_id:"post-offline-1",user_id:"demo-alex",user_name:"Alex Rivera",user_avatar_url:null,content:"Thanks, Sam. I am also testing a monthly review format.",timestamp:new Date(r-36e6).toISOString()},{id:"comment-3",post_id:"post-offline-2",user_id:"demo-alex",user_name:"Alex Rivera",user_avatar_url:null,content:"Good checklist. The muted-text rule avoids so many contrast issues.",timestamp:new Date(r-108e5).toISOString()}];e.transaction(()=>{let t=e.prepare(`INSERT OR REPLACE INTO users (id, name, email, password, avatar_url, bio, created_at)
       ...
       VALUES (@id, @post_id, @user_id, @user_name, @user_avatar_url, @content, @timestamp)`);a.forEach(e=>t.run(e)),i.forEach(e=>r.run(e)),s.forEach(e=>o.run(e)),e.prepare(`UPDATE posts
       SET comment_count = (
...
`;R();let L={getUserById(e){R();let t=c().prepare("SELECT id, name, email, avatar_url, bio, password FROM users WHERE id = ?").get(e);return t?S(t):null},loginWithEmail(e,t){R();let r=c().prepare("SELECT id, name, email, avatar_url, bio, password FROM users WHERE email = ?").get(e.toLowerCase().trim());if(!r||r.password!==t)throw Error("Invalid email or password.");return S(r)},signup(e){R();let t=c(),r=e.email.toLowerCase().trim();if(t.prepare("SELECT id FROM users WHERE email = ?").get(r))throw Error("An account with this email already exists.");let a=g();return t.prepare(`INSERT INTO users (id, name, email, password, avatar_url, bio, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`).run(a,e.name.trim(),r,e.pass,null,"Newly registered user.",_()),this.getUserById(a)},updateUserProfile(e,t){R();let r=c(),a=r.prepare("SELECT id, name, email, avatar_url, bio, password FROM users WHERE id = ?").get(e);if(!a)throw Error("User not found.");let i=t.name?.trim()||a.name,s=void 0!==t.bio?t.bio:a.bio,o=void 0!==t.avatarUrl?t.avatarUrl||null:a.avatar_url,n=t.email?.trim().toLowerCase()||a.email;if(n!==a.email){let t=r.prepare("SELECT id FROM users WHERE email = ?").get(n);if(t&&t.id!==e)throw Error("An account with this email already exists.")}return r.prepare(`UPDATE users
       SET name = ?, email = ?, bio = ?, avatar_url = ?, updated_at = ?
       WHERE id = ?`).run(i,n,s,o,_(),e),this.getUserById(e)},getAllPosts(e){R();let t=c(),r=[],a=[];e?.authorId&&(r.push("p.author_id = ?"),a.push(e.authorId)),e?.status&&(r.push("p.status = ?"),a.push(e.status));let i=r.length?`WHERE ${r.join(" AND ")}`:"";return t.prepare(`${I()} ${i} ORDER BY p.timestamp DESC`).all(...a).map(h)},getPostById(e){R();let t=c().prepare(`${I()} WHERE p.id = ? LIMIT 1`).get(e);return t?h(t):void 0},addPost(e,t){R();let r=c(),a=g();return r.prepare(`INSERT INTO posts (id, title, excerpt, content, author_id, status, timestamp, categories, tags, image_url, comment_count)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)`).run(a,e.title,e.excerpt,e.content,t.id,e.status,_(),JSON.stringify(e.categories),JSON.stringify(e.tags),e.imageUrl??null),this.getPostById(a)},updatePost(e){R();let t=c(),r=this.getPostById(e.id);if(!r)return;let a=e.title??r.title,i=e.excerpt??r.excerpt,s=e.content??r.content,o=e.status??r.status,n=e.categories??r.categories,u=e.tags??r.tags,l=void 0!==e.imageUrl?e.imageUrl:r.imageUrl;return t.prepare(`UPDATE posts
       SET title = ?, excerpt = ?, content = ?, status = ?, categories = ?, tags = ?, image_url = ?, last_modified_at = ?
       WHERE id = ?`).run(a,i,s,o,JSON.stringify(n),JSON.stringify(u),l??null,_(),e.id),this.getPostById(e.id)},deletePost(e){R();let t=c();t.transaction(()=>{t.prepare("DELETE FROM comments WHERE post_id = ?").run(e),t.prepare("DELETE FROM posts WHERE id = ?").run(e)})()},getCommentsByPostId:e=>(R(),c().prepare(`SELECT id, post_id, user_id, user_name, user_avatar_url, content, timestamp
         FROM comments
         WHERE post_id = ?
         ORDER BY timestamp DESC`).all(e).map(O)),addComment(e,t){R();let r=c(),a=g();return r.transaction(()=>{r.prepare(`INSERT INTO comments (id, post_id, user_id, user_name, user_avatar_url, content, timestamp)
         VALUES (?, ?, ?, ?, ?, ?, ?)`).run(a,e.postId,t.id,t.name,t.avatarUrl??null,e.content,_()),r.prepare("UPDATE posts SET comment_count = comment_count + 1 WHERE id = ?").run(e.postId)})(),O(r.prepare(`SELECT id, post_id, user_id, user_name, user_avatar_url, content, timestamp
         FROM comments
         WHERE id = ?`).get(a))},deleteComment(e,t){R();let r=c();r.transaction(()=>{let a=r.prepare("SELECT id, post_id FROM comments WHERE id = ?").get(t);a&&a.post_id===e&&(r.prepare("DELETE FROM comments WHERE id = ?").run(t),r.prepare("UPDATE posts SET comment_count = MAX(comment_count - 1, 0) WHERE id = ?").run(e))})()}}},29294:e=>{"use strict";e.exports=require("next/dist/server/app-render/work-async-storage.external.js")},43775:(e,t,r)=>{"use strict";r.r(t),r.d(t,{patchFetch:()=>c,routeModule:()=>d,serverHooks:()=>E,workAsyncStorage:()=>m,workUnitAsyncStorage:()=>p});var a={};r.r(a),r.d(a,{POST:()=>l});var i=r(96559),s=r(48088),o=r(37719),n=r(32190),u=r(26346);async function l(e){try{let t=await e.json(),r=u.w.loginWithEmail(t.email,t.pass);return n.NextResponse.json({user:r})}catch(e){return n.NextResponse.json({error:e.message||"Login failed"},{status:400})}}let d=new i.AppRouteRouteModule({definition:{kind:s.RouteKind.APP_ROUTE,page:"/api/auth/login/route",pathname:"/api/auth/login",filename:"route",bundlePath:"app/api/auth/login/route"},resolvedPagePath:"/home/ritvik/code/Blog-Sphere/src/app/api/auth/login/route.ts",nextConfigOutput:"",userland:a}),{workAsyncStorage:m,workUnitAsyncStorage:p,serverHooks:E}=d;function c(){return(0,o.patchFetch)({workAsyncStorage:m,workUnitAsyncStorage:p})}},44870:e=>{"use strict";e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},63033:e=>{"use strict";e.exports=require("next/dist/server/app-render/work-unit-async-storage.external.js")},76760:e=>{"use strict";e.exports=require("node:path")},78335:()=>{},96487:()=>{}};var t=require("../../../../webpack-runtime.js");t.C(e);var r=e=>t(t.s=e),a=t.X(0,[447,580],()=>r(43775));module.exports=a})();